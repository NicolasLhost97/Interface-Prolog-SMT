...
:- dynamic model_constraint_counter/1.
model_constraint_counter(0).
...
% They retrieve the new script with new constraints from the model if asked
    ...
    smt_solve(Stream, Solver) :-
        ...
        retract(solve_counter(SolveCounter)),
        NewSolveCounter is SolveCounter + 1,
        asserta(solve_counter(NewSolveCounter)),
        % Extract constraints
        extract_funs_from_model_to_constraints(Expressions, SolveCounter, ModelConstraints),
        % Reset the model_constraint_counter
        retractall(model_constraint_counter(_)),
        asserta(model_constraint_counter(0)),
        % write('\n\n Constraints'),
...
% SCRIPTS PREDICATES
    ...
    smt_get_model_to_constraint_for(Symbols,Stream) :-
        % Increment the model_constraint_counter
        retract(model_constraint_counter(Counter)),
        NewCounter is Counter + 1,
        asserta(model_constraint_counter(NewCounter)),
        % Create all commands
